...
class _CompletarPerfilPasajeroState extends State<CompletarPerfilPasajero> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _pulseController = AnimationController(
      ...
      upperBound: 1.15,
        ...
        else if (status == AnimationStatus.dismissed && _isListening) _pulseController.forward();
      });
  }
  ...
  TextStyle mBold(BuildContext context, {Color color = Colors.black, double size = 11}) =>
      GoogleFonts.montserrat(color: color, fontSize: sp(size, context), fontWeight: FontWeight.bold);

  TextStyle mExtrabold(BuildContext context, {Color color = Colors.black, double size = 14}) =>
      GoogleFonts.montserrat(color: color, fontSize: sp(size, context), fontWeight: FontWeight.w700);
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(child: _buildDocCard(context, 'Anverso', _ineAnverso, 'assets/ine_anverso.png', 'anverso')),
                          const SizedBox(width: 15),
                          Expanded(child: _buildDocCard(context, 'Reverso', _ineReverso, 'assets/ine_reverso.png', 'reverso')),
                        ],
                      ),
                      const SizedBox(height: 35),
                      Text('¿Presenta alguna necesidad especial?', style: mExtrabold(context, size: 17)),
                      Text('Seleccione las casillas que se ajusten a su necesidad', style: mBold(context, color: Colors.red, size: 10)),
                      const SizedBox(height: 25),
                      ...
                      Align(
                        ...
                        child: ElevatedButton(
                          ...
                          child: Text('Registrar un acompañante', style: mBold(context, color: Colors.white, size: 14)),
                        ),
                      ),
                      const SizedBox(height: 40),
                    ],
      ...
      children: [
        ...
        Positioned(
          ...
          child: Column(
            ...
            children: [
              Text('Username', style: GoogleFonts.montserrat(fontSize: 22, fontWeight: FontWeight.w900)),
              Row(
                children: [
                  ...
                  Text('5.00', style: mBold(context, color: primaryBlue, size: 12)),
                ],
              ),
  ...
  Widget _buildStatusButton(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: statusRed, borderRadius: BorderRadius.circular(20)),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.error_outline, color: Colors.white, size: 20),
          const SizedBox(width: 8),
          Text('Completar perfil', style: mBold(context, color: Colors.white, size: 14)),
        ],
      ),
    );
  }

  Widget _buildDocCard(BuildContext context, String label, File? file, String placeholder, String type) {
    return Column(
      children: [
        GestureDetector(
  ...
  Widget _buildNeedsGrid(BuildContext context) {
    ...
    return Wrap(
      spacing: 12,
      runSpacing: 20,
      children: needs.map((n) => _buildNeedItem(context, n['label']!, n['icon']!)).toList(),
    );
  }
  ...
  Widget _buildNeedItem(BuildContext context, String label, String iconPath) {
    ...
    return GestureDetector(
      onTap: () => setState(() => isSelected ? _selectedNeeds.remove(label) : _selectedNeeds.add(label)),
      child: SizedBox(
        width: 100, 
        child: Column(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: Colors.white,
                border: Border.all(color: isSelected ? primaryBlue : accentBlue, width: isSelected ? 3 : 1.5),
  ...
  Future<void> _pickImage(String type) async {
    ...
    if (image != null) {
      setState(() {
        if (type == 'anverso') _ineAnverso = File(image.path);
        else _ineReverso = File(image.path);
      });
    }
  }
  ...
  Widget _buildCustomBottomNav(BuildContext context) {
    return Container(
      height: 70, color: const Color(0xFFD6E8FF),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: List.generate(4, (i) => _navIcon(i, [Icons.home, Icons.location_on, Icons.history, Icons.person][i])),
      ),
    );
  }
